...
DECLINED_DOMAINS = [{'domain': 'lifecycle',
 ...
 {'domain': 'flow_repeatability',
  'reason': "the study this domain asks for is a distribution over the declared route search, and the search this board declares does not reproduce this board under the router the tree carries: run from the generated placement, all three declared orderings were refused, each because the router's own signal copper split the V24 pour into islands the DRC then counted as unconnected. The committed copper was accepted by KiCadRoutingTools 0.21.5 under KiCad 10.0.5, and the tree carries 0.22.0 under 10.0.6, so a study run now would sample a search that does not produce the copper being released and would report a distribution over a different flow. The copper itself is judged by measurement on every run - a fresh DRC, ERC and schematic parity over the board in the tree - and ROUTE.PROVENANCE holds it to the record that accepted it; what cannot be stated today is the accept rate. Protecting the pour's neck so the search reproduces the board is the next revision's work and is named here rather than left silent"},
